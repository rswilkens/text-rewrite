...
public class NodePattern extends SemgrexPattern  {
	...
	/**
	 * All features with subvalues (e.g. feats) should be identified 
	 */
	private static List<String> mappedFeatures = new ArrayList<String>(Arrays.asList("conllfeats".split(" ")));


	public static boolean addMappedFeature(String e) {
		return mappedFeatures.add(e);
	}

	public static void clearMappedFeatures() {
	...
	public NodePattern(GraphRelation r, boolean negDesc,
		...
		for (Map.Entry<String, String> entry : attrs.entrySet()) {
			...
			if (value.equals("__")) {
				...
				if(mappedFeatures.contains(key)) {
					boolean isRegexp = false;
					value = value.substring(1, value.length() - 1);
					String[] items = value.split(";");
					Map<String, Pair> aux = new HashMap<String, Pair>();
					for (String item : items) {
						String[] cols = item.split("=");
						cols[0] = key +":" + cols[0];
						for (int i = 0; i < cols[1].length(); ++i) {
							char chr = cols[1].charAt(i);
						...
						if (isRegexp) {
							aux.put(cols[0], Pair.makePair(
									Pattern.compile(cols[1]),
									Pattern.compile(cols[1], Pattern.CASE_INSENSITIVE|Pattern.UNICODE_CASE))
									);
						} else {
							aux.put(cols[0], Pair.makePair(cols[1], cols[1]));
						}
					}
					attributes.put(key, Pair.makePair(aux,aux)); //TODO it's ugly but it works @rswilkens
...
//					}        		
	...
	public boolean nodeAttrMatch(IndexedWord node, final SemanticGraph sg, boolean ignoreCase) {
		...
		for (Map.Entry<String, Pair<Object, Object>> attr : attributes.entrySet()) {
			...
			boolean matches = matches(ignoreCase, attr, nodeValue, value);
	...
	private boolean matches(boolean ignoreCase, Entry<String, Pair<Object, Object>> attr, String nodeValue, Object value) {
		...
		if (toMatch instanceof Boolean) {
			...
			if(value instanceof Map) {
				...
				nodeValue = "";
				if(k.contains(":")){
					Object val = ((Map)value).get(k.split(":")[1]);
					if(val!=null)
						nodeValue = val.toString();
				}else
					new Exception("malformed term exception (" + k + ").");
			}
			if (ignoreCase) {
			...
			if(nodeValue.length()>0)
				...
				matches = false; //@rswilkens if there is not the feature in the map we reject the patters due to the principle that some value was requested 
		} else if (toMatch instanceof Map) {  //TODO @rswilkens change it in a way to allow several fields
			matches = !((Map)toMatch).isEmpty(); //we cannot accept an empty toMatch; if it's empty the assessment goes to each subfeature   
			
			for (Object k : ((Map)toMatch).entrySet()) {//k=conllfeats
				Entry<String, Pair<Object, Object>> kc = (Entry) k;
				matches = matches(ignoreCase, kc, nodeValue, value) & matches; //TODO @rswilkens matches==false -> break;
			}
		} else {
