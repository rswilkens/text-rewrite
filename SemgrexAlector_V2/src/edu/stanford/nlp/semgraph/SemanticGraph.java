...
public class SemanticGraph implements Serializable  {
	...
	public String toString(CoreLabel.OutputFormat wordFormat) {
		...
		for (IndexedWord root : rootNodes) {
			if(wordFormat.equals(CoreLabel.OutputFormat.CoNLL) || wordFormat.equals(CoreLabel.OutputFormat.CoNLLcoref))
				sb.append(root.toString(wordFormat)).append("\n");
			else
		...
		if(wordFormat.equals(CoreLabel.OutputFormat.CoNLL) || wordFormat.equals(CoreLabel.OutputFormat.CoNLLcoref)) { //sort output by word index
			...
			for (String ln : lns) {
//				ln = ln.replace("-> ", "").trim();
				lines.set(Integer.valueOf(ln.split("\t")[0])-1, ln);
			}
			StringBuffer out = new StringBuffer();
			for (String l : lines) 
				out.append(l)/*.append("\t_\t_")*/.append("\n");
			ret = out.toString();
		}
		return ret;
	}
	...
	private void recToString(IndexedWord curr, CoreLabel.OutputFormat wordFormat, StringBuilder sb, int offset, Set<IndexedWord> used) {
		...
		for (SemanticGraphEdge edge : edges) {
			...
			if(wordFormat.equals(CoreLabel.DEFAULT_FORMAT.CoNLL) || wordFormat.equals(CoreLabel.OutputFormat.CoNLLcoref))
				sb.append(target.toString(wordFormat)).append("\n");
			else
