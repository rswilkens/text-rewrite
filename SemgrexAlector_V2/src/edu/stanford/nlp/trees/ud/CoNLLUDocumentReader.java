  ...
  private static class WordProcessor implements Function<String,IndexedWord> {
    ...
    public IndexedWord apply(String line) {
      ...
      if (bits[0].contains("-")) {
        ...
        /* Parse features. */
        HashMap<String, String> features = CoNLLUUtils.parseFeatures(bits[5].replace(".", "")); //TODO rswilkens
        ...
        word.set(CoreAnnotations.PartOfSpeechAnnotation.class, bits[3]);//rswilkens indexes 3 and 4 inverted

        word.set(CoreAnnotations.CoNLLDepParentIndexAnnotation.class, Integer.parseInt(bits[6]));
        word.set(CoreAnnotations.CoNLLDepTypeAnnotation.class, bits[7].replace(".", "").replace("_", ""));
        ...
        /* Parse features. */
        HashMap<String, String> features = CoNLLUUtils.parseFeatures(bits[5].replace(".", ""));
        word.set(CoreAnnotations.CoNLLUFeats.class, features); //TODO rswilkens
        
        if(bits.length==11) {
        	word.set(CoreAnnotations.CoNLLCorefAnnotation.class, bits[10]); //TODO rswilkens
        }
