...
public class ShowResults extends JFrame {
	...
	private Map<String, List<Pair<String, Map<String, String>>>> allCorporaProcessed;
	private Map<String, Map<String, String>> descCorpus = new HashMap<>();
	private Map<String, String> descSents = new HashMap<>();
	private DefaultComboBoxModel<String> operationModel;

	public ShowResults(Map<String, List<Pair<String, Map<String, String>>>> map) {
		...
		cmbCorpus.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				...
				for (Pair<String, Map<String, String>> vals : map.get(selectedItem.toString())) {
		...
		cmbSentences.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				if (item != null) {
					...
					Map<String, String> ops = descCorpus.get(item.toString());
					operationModel.removeAllElements();
					descSents = new Hashtable<String, String>();
					for (String op : ops.keySet()) {
						operationModel.addElement(op);
						descSents.put(op, ops.get(op));
					}
					String lst = descSents.get(operationModel.getSelectedItem().toString());
					StringBuffer aux = new StringBuffer();
					if(lst != null) {
						aux.append("# sentence = " + item.toString() + "\n");
						aux.append(lst);
						aux.append("\n########################################################\n");
...
//						}
		...
		cmbOperations.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				if (item != null) {
					String lst = descSents.get(item.toString());
					StringBuffer aux = new StringBuffer();
					if(lst != null) {
						aux.append("# sentence = " + item.toString() + "\n");
						aux.append(lst);
...
//						}
		...
		for (Pair<String, Map<String, String>> vals : allCorporaProcessed.get(selectedItem.toString())) {
			descCorpus.put(vals.first(), vals.second());
			sentModel.addElement(vals.first());
		}
		Map<String, String> ops = descCorpus.get(cmbSentences.getItemAt(0).toString());
		operationModel.removeAllElements();
		descSents = new Hashtable<String, String>();
		for (String op : ops.keySet()) {
			operationModel.addElement(op);
		...
		String lst = descSents.get(operationModel.getElementAt(0).toString());
		StringBuffer aux = new StringBuffer();
		if(lst != null) {
			aux.append("# sentence = " + operationModel.getElementAt(0).toString() + "\n");
			aux.append(lst);
			aux.append("\n########################################################\n");
//			for (SemanticGraph semanticGraph : lst) {
//				aux.append("# sentence = " + semanticGraph.toRecoveredSentenceString() + "\n");
